...
fn model(app: &App) -> Model {
    ...
    for _ in 0..600 {
        points.push(Point {
            ...
            color: u32_to_srgba(colors[random_range(0, colors.len())], random_range(0.0, 1.0)),
            resolution: 5.0,
        });
        t += 0.05;
    }
...
fn update(app: &App, model: &mut Model, _update: Update) {
    for point in &mut model.points {
        ...
        point.radius += 0.01;
        //point.resolution = map_range(t.sin(), -1.0, 1.0, 1.0, 10.0); //random_range(1.0, 10.0);
        point.resolution = map_range(point.noise.0.sin(), -1.0, 1.0, 1.0, 10.0); //random_range(1.0, 10.0);
        //point.resolution = map_range(point.noise.0.sin(), -1.0, 1.0, 3.0, 10.0); //random_range(1.0, 10.0);
        point.noise.0 += 0.01;
        point.noise.1 += 0.01;
    }
}
